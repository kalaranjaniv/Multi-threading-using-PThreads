...
void main(int argc, char **argv) {
    ...
    long i;
  A=(float**) malloc(MAXN*sizeof(float));
   for(i=0;i<MAXN;i++)
{
A[i]=(float *) malloc(MAXN * sizeof(float));
}
...
void gauss() {
    long i;
   pthread_t thread_id[procs];
         /* Normalization row, and zeroing			* element row a
  * m ultiplier=(float**) malloc(N*sizeof(float));
    for(i=0;i<N;i++)
   {
multiplier[i]=(float *) malloc(N*sizeof(float));
}*/
    /* Gaussian elimination */
    for (norm =0 ; norm < N - 1; norm++) 
         ...
         for(i=1;i<=procs;i++)
          {
           pthread_create(&thread_id[i],NULL,(void *)&subcode,(void *)(norm+i));
          }
         for(i=1;i<=procs;i++)
          {
